...
class SessaoUser(HttpUser):
    ...
    def prepare_data(self):
        ...
        s_xml = f"<?xml version='1.0' encoding='UTF-8'?><sala><nome>Sala_{fake.random_int(1, 999999)}</nome><capacidade>100</capacidade></sala>"
        ...
        if self.filme_id and self.sala_id:
            h_xml = f"<?xml version='1.0' encoding='UTF-8'?><homologacao><idFilme>{self.filme_id}</idFilme><idSala>{self.sala_id}</idSala><requisitoTecnico>2D</requisitoTecnico><statusValidacao>Aprovado</statusValidacao></homologacao>"
            self.client.post("/homologacoes", data=h_xml, headers=auth_h, catch_response=True)

    @task
    def ciclo_completo_sessao(self):
        ...
        dt = (datetime.now() + timedelta(days=fake.random_int(1, 30))).strftime("%Y-%m-%dT%H:%M:%S")

        xml = f"<?xml version='1.0' encoding='UTF-8'?><sessao><idFilme>{self.filme_id}</idFilme><idSala>{self.sala_id}</idSala><dataHora>{dt}</dataHora><precoBase>30.00</precoBase><tipoExibicao>2D</tipoExibicao></sessao>"

        with self.client.post("/sessoes", data=xml, headers=headers, catch_response=True, name="POST /sessoes") as response:
            if response.status_code == 201:
                location = response.headers.get("Location")
                if location:
                    self.client.get(location, headers=headers, name="GET /sessoes/[id]")
            else:
